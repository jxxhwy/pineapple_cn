registerController("ConfigurationGeneralController", ['$api', '$scope', '$timeout', function($api, $scope, $timeout) {
		...
		if (confirm("您确定要关闭设备吗？")) {
			$api.request({
				...
				if (response.success !== undefined) {
					$scope.actionMessage = "您的设备正在关闭中， 请在LED熄灭后再断开电源。";
					$timeout(function(){
					    $scope.actionMessage = "";
					}, 10000);
				}
	...
	$scope.rebootPineapple = (function() {
		if (confirm("您确定要重启设备吗?")) {
			$api.request({
				...
				if (response.success !== undefined) {
					$scope.actionMessage = "您的设备正在重启中，成功后，您可能需要重新连接。";
					$timeout(function(){
					    $scope.actionMessage = "";
					}, 10000);
				}
	...
	$scope.resetPineapple = (function() {
		if($scope.device === 'nano') {
			$scope.resetMessage = "您确定要工厂重置设备吗？\n\n这将会清除SD卡上所有数据。";
		...
		if (confirm($scope.resetMessage)) {
			$api.request({
				...
				if (response.success !== undefined) {
					$scope.actionMessage = "您的设备正在恢复为出厂默认设置。这可能需要几分钟时间，您将会被断开连接。";
					$timeout(function(){
					    $scope.actionMessage = "";
					}, 10000);
				}
