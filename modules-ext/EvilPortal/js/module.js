registerController("EvilPortalController", ['$api', '$scope', function ($api, $scope) {
    ...
    $scope.controls = [
        { "title": "门户认证", "visible": true, "throbber": false, "status": "Start"},
        {"title": "启动运行", "visible": true, "throbber": false, "status": "Enable"}
    ];
    ...
    $scope.createNewPortal = function(storage) {
        $api.request({
            ...
            if (!response.success) {
                $scope.sendMessage('创建门户时发生错误:', response.message);
                return;
            }
            $scope.newPortal = {"type": "basic", "name": ""};
            getPortals();
    ...
    function updateControls() {
        $scope.controls = [
            {
                "title": "门户认证",
                "status": ($scope.evilPortal.running) ? "停止" : "启动",
                "visible": true,
                "throbber": false
            },
            {
                "title": "启动运行",
                "status": ($scope.evilPortal.startOnBoot) ? "禁止": "允许",
                "visible": true,
                "throbber": false
            }
        ];
